...
K6 can be installed with the following command (at least on Windows):
```bash
...
Swagger documentation for this project can be generated by running the following command in the terminal (assuming Swagger is installed):
```bash
...
This project uses Uber-Fx for Dependency Injection (DI).  While a DI framework is overkill for a project this small, I did want to try out a GoLang DI framework.

To install Fx, run the following in the project root directory:
```bash
...
## Solution TODO List:
* Replace Postman with a different tool.  Bruno?  Scalar?
* Replace Swagger with an OpenAPI tool that works with GoLang.
* Add first proper endpoint (puzzle validity?).
* Add K6 testing suite.
* Add the actual puzzle solver algorithm.
* Add web page for users to interact with.
* Add separate data access app.
* Add a DB (Postgres).
* Add a cache (Redis).
* Fully "containerise" the project (Docker).
